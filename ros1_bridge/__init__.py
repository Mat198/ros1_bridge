    ...
    def __init__(self, data, expected_package_name):
        ...
        if all(n in data for n in ('ros1_message_name', 'ros2_message_name')):
            ...
            if 'fields_1_to_2' or 'fields_2_to_1' in data:
                ...
                print("Expecting ", 4 + int('enable_foreign_mappings' in data), 
                      "fields.\n Data:", data)
                raise RuntimeError('Mapping for package %s messages contains unknown field(s)' 
                                   % self.ros2_package_name)
        elif len(data) > 2 + int('enable_foreign_mappings' in data):
            print("Expecting ", 2 + int('enable_foreign_mappings' in data), 
                  "fields.\n Data:", data)
            raise RuntimeError('Mapping for package %s messages contains unknown field(s)' 
                               % self.ros2_package_name)
    ...
    def __init__(self, data, expected_package_name):
        ...
        if all(n in data for n in ('ros1_service_name', 'ros2_service_name')):
            ...
            if 'response_fields_1_to_2' in data:
                ...
                print("Expecting ", 4 + int('enable_foreign_mappings' in data), 
                      "fields.\n Data:", data)
                raise RuntimeError('Mapping for package %s service contains unknown field(s)'
                                    % self.ros2_package_name)
        elif len(data) > 2 + int('enable_foreign_mappings' in data):
            print("Expecting ", 2 + int('enable_foreign_mappings' in data), 
                  "fields.\n Data:", data)
    ...
    def __init__(self, data, expected_package_name):
        ...
        if all(n in data for n in ('ros1_action_name', 'ros2_action_name')):
            ...
            if 'feedback_fields_1_to_2' in data:
                ...
                print("Expecting ", expected_keys + int('enable_foreign_mappings' in data), 
                      "fields.\n Data:", data, file=sys.stderr)
                raise RuntimeError('Mapping for package %s actions contains unknown field(s).' 
                                   % self.ros2_package_name)
        elif len(data) > 2 + int('enable_foreign_mappings' in data):
            print("Expecting", 2 + int('enable_foreign_mappings' in data), 
                  "fields.\n Data:", data, file=sys.stderr)
            raise RuntimeError(
                'Mapping for package %s actions contains unknown field(s).'
                % self.ros2_package_name)
...
def determine_message_pairs(ros1_msgs, ros2_msgs, package_pairs, mapping_rules):
    ...
    for rule in mapping_rules:
        print("Found mapping rule: \n  - %s <-> %s \n    - %s <-> %s"
              % (rule.ros1_package_name, rule.ros2_package_name, 
                 rule.ros1_message_name, rule.ros2_message_name))
        ...
        for ros1_msg in ros1_msgs:
            ...
            if rule.ros1_message_name: 
                print("  Couldn't find correspondence for ROS 1 message rule." \
                    "\n    - %s <-> %s\n      - %s <-> %s"
                    % (rule.ros1_package_name, rule.ros2_package_name, 
                        rule.ros1_message_name, rule.ros2_message_name), file=sys.stderr)
                print("  Available msgs:")
                for ros1_msg in ros1_msgs:
                    if ros1_msg.package_name == rule.ros1_package_name:
                        print("    - ", ros1_msg.message_name)
        ...
        for ros2_msg in ros2_msgs:
            ...
            if rule.ros2_message_name: 
                print("  Couldn't find correspondence for ROS 2 message rule." \
                    "\n    - %s <-> %s\n      - %s <-> %s"
                    % (rule.ros1_package_name, rule.ros2_package_name, 
                        rule.ros1_message_name, rule.ros2_message_name), file=sys.stderr)
                print("  Available msgs:")
                for ros2_msg in ros2_msg:
                    if rule.ros2_package_name == ros2_msg.package_name:
                        print("    - ", ros2_msg.message_name)
                # skip unknown messages
            continue
        print("  Found pair: %s <-> %s !" % (ros1_msg.message_name, ros2_msg.message_name))
...
def determine_service_pairs(ros1_srvs, ros2_srvs, package_pairs, mapping_rules):
    ...
    for rule in mapping_rules:
        print("Found mapping rule: \n  - %s <-> %s \n    - %s <-> %s"
              % (rule.ros1_package_name, rule.ros2_package_name, 
                 rule.ros1_service_name, rule.ros2_service_name))
        for ros1_srv in ros1_srvs:
            if rule.ros1_package_name == ros1_srv.package_name and \
                    rule.ros1_service_name == ros1_srv.message_name:
                break
        else:
            if rule.ros1_service_name:
                print("  Couldn't find correspondence for ROS 1 service rule." \
                    "\n    - %s <-> %s\n      - %s <-> %s"
                    % (rule.ros1_package_name, rule.ros2_package_name, 
                        rule.ros1_service_name, rule.ros2_service_name), file=sys.stderr)
                print("  Available services:")
                for ros1_srv in ros1_srvs:
                    if ros1_srv.package_name == rule.ros1_package_name:
                        print("    - ", ros1_srv.message_name)
                # skip unknown messages
            continue
        for ros2_srv in ros2_srvs:
            ...
            if rule.ros2_service_name:
                print("  Couldn't find correspondence for ROS 2 service rule." \
                    "\n    - %s <-> %s\n      - %s <-> %s"
                    % (rule.ros1_package_name, rule.ros2_package_name, 
                        rule.ros1_service_name, rule.ros2_service_name), file=sys.stderr)
                print("  Available msgs:")
                for ros2_srv in ros2_srvs:
                    if rule.ros2_package_name == ros2_srv.package_name:
                        print("    - ", ros2_srv.message_name)
                # skip unknown messages
            continue
        pair = (ros1_srv, ros2_srv)
        if pair not in pairs:
            print("  Found pair: %s <-> %s !" % (ros1_srv.message_name, ros2_srv.message_name))
            pairs.append(pair)
...
def determine_common_services(
    ...
    print("\n###### Creating bridges for services ######\n")
    if message_string_pairs is None:
        message_string_pairs = set()

    package_pairs = determine_package_pairs(ros1_srvs, ros2_srvs, mapping_rules)
    srv_pairs = determine_service_pairs(ros1_srvs, ros2_srvs, package_pairs, mapping_rules)
    ...
    for pair in pairs:
        ...
        for direction in ['request', 'response']:
            if len(ros1_fields[direction]) != len(ros2_fields[direction]):
                print("Failed to match %s<-->%s different fields" %(ros1_name, ros2_name), file=sys.stderr)
                match = False
                break
                ...
                if ros1_name != ros2_name:
                    ...
                    if not ros1_is_builtin or not ros2_is_builtin or ros1_type != ros2_type:
                        ...
                        if ((ros1_type, ros2_type) not in message_string_pairs and
                            ...
                            match = False
                            print("Failed to match %s<-->%s" %(ros1_name, ros2_name), file=sys.stderr)
                            break
        ...
        for ros2_member in ros2_spec.structure.members:
            for ros1_field in ros1_spec.parsed_fields():
                ...
                if not allow_missing:
                    print("Error generating message from pkg %s <-> %s\n" 
                          "  - Message %s <-> %s has non mappable field '%s'!" 
                          % (ros1_msg.package_name, ros1_msg.message_name, 
                             ros1_msg.message_name, ros2_msg.message_name, ros1_field.name), 
                             file=sys.stderr)
                    return None
    # except Exception as e:
